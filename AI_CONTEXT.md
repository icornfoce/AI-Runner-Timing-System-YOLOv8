...
| `MAX_SCAN_WIDTH` | `1920` | **Detection-only** resolution cap (2026-05-21 perf pass). Photos WIDER than this are detected on a separate downscaled offscreen canvas inside `processPhotoWithBytes`; the resulting boxes are scaled back to native coords via `faceapi.resizeResults`. The `#preview-canvas` (OCR source for `runOCRForFace` **and** the frame of reference for persisted `detectionBoxes` coords) stays full-resolution, so **OCR accuracy and the dashboard evidence-modal overlay are unchanged**. Saves `drawImage` + tensor-upload time on 4K event shots (SSD MobileNet resamples to ~600 px internally, so the full-res detect input was wasted work). Width-gated to match landscape event shots. Trade: face descriptors for photos above the cap are computed from the downscaled face (still large at 1920 px; `FACE_MATCH_DISTANCE = 0.45` absorbs it) — verify identification on a few 4K shots if raising the cap's effect is in doubt. Photos at/below the cap are detected natively (no extra work). |
